...
import com.wyc.listener.PayResultListener;
...
public class PayResultHandler implements Handler{
    ...
    public Object handle(HttpServletRequest httpServletRequest)
    	...
    	try{
			payResultListener.prepareHandle(tempGroupOrder);
		}catch(Exception e){
			logger.error("has an error of prepare handler:{}",e);
		}
    	...
    	try{
            ...
            GroupPartakeDeliver groupPartakeDeliver = null;
            ...
            //    Good good = goodService.findOne(tempGroupOrder.getGoodId());
                ...
                goodGroup.setAllowInsteadOfRelief(tempGroupOrder.getAllowInsteadOfRelief());
                goodGroup.setForceInsteadOfRelief(tempGroupOrder.getForceInsteadOfRelief());
                goodGroup.setInsteadOfRelief(tempGroupOrder.getInsteadOfRelief());
                goodGroup.setReceiverInsteadOfRelief(tempGroupOrder.getReceiverInsteadOfRelief());
                ...
                groupPartakeDeliver = new GroupPartakeDeliver();
            ...
            try{
    			payResultListener.paySuccess(customer,goodGroup,goodOrder,openGroupCoupon,groupPartake,groupPartakeDeliver,orderDetail);
    		}catch(Exception e){
    			logger.error("has an error of prepare handler:{}",e);
    		}
    	}catch(Exception e){
    		payResultListener.payFailure(tempGroupOrder);
    		throw e;
    	}
